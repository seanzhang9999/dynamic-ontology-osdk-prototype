...
| 数据运营 Agent | 数据提供方内部的技术型 Agent，能受控调用目录、数据平台、质量、编译、部署和审计工具，但关键动作需要人工确认 |
| OntoFlow / 动态本体平台 | OSDK 的上游建模和生产能力，可作为 awiki.ai Agent 网络中的可操作 skill 对象 |
...
| 上游本体平台可以被纳入生态 | OntoFlow 或其他动态本体平台负责编排建模、加工和实例化，输出 DOIR 或产品投影供 OSDK 编译 |
...
### 5.4 数据运营 Agent 需要什么能力边界

数据交易服务的数字员工不是普通客服机器人。它们要真正降低数据提供方的工作强度，就必须具备更多全栈技术访问能力：能看数据目录、读元数据、跑质量检查、调用动态本体平台、触发产品编译、检查 Runtime Binding、提交发布申请、跟踪任务、读取审计日志。

但这不意味着 Agent 可以自由访问生产数据或自由发布数据产品。正确边界是：

```text
内部生产侧：Agent 可以受控调用数据平台、动态本体平台、质量平台、编译器和运维工具；
外部服务侧：Agent 只能通过 Product OSDK / MCP Tool / Gateway 调用已发布的数据产品；
高风险动作：必须有人类审批或双人复核，不能完全交给 Agent 自主决定。
```

这也是“OSDK 向外、平台向内”的分工：

| 场景 | Agent 应该用什么能力 | 为什么 |
| --- | --- | --- |
| 发现源表、字段、口径和血缘 | 数据目录、数据平台、动态本体平台 API | 这是数据提供方内部生产工作，需要充分利用现有平台能力 |
| 建模、映射、分类分级建议 | OntoFlow / 动态本体平台、LLM 辅助分析、规则库 | 需要理解底层结构和业务语义，不能只靠已发布 OSDK |
| 质量检查和样例验证 | 质量平台、DuckDB/SQL、GIS/图计算、测试数据集 | 这是产品发布前的内部质检 |
| 产品编译和接口收缩 | Product Compiler / OSDK Generator | 把内部能力裁剪成可授权的数据产品接口 |
| 发布、授权、撤销、外发结果 | Policy、Gateway、Audit、Receipt | 这是可信数据空间边界，必须强制执行 |
| 买方或外部 Agent 调用产品 | Product OSDK / MCP Tool | 外部 Agent 不应直接碰底层表、文件、SQL 或连接串 |

因此，数据运营 Agent 的权限要按动作风险分层：

| 风险级别 | 可自动执行 | 必须人工确认 |
| --- | --- | --- |
| 低风险 | 读取元数据、生成说明、草拟映射、生成测试报告 | 通常不需要 |
| 中风险 | 运行质量任务、生成产品草案、发起编译、模拟授权 | 发布前需要业务或数据负责人确认 |
| 高风险 | 发布产品、扩大可见范围、批准特殊用途、撤销关键授权 | 必须人工审批 |
| 禁止默认自动化 | 导出原始数据、修改生产源数据、绕过 Policy、直接签发 Receipt | 不应开放给 Agent |

这会让“数据服务数字员工部”更可信：Agent 有足够工具完成繁琐技术工作，但不能越过人的责任边界。
...
| 动态本体建模平台 | 不必替代 | OntoFlow / 其他动态本体平台 / 数据平台能力 |
...
### 5.7 OntoFlow / 动态本体平台在技术框架中的位置

从技术框架上，我们应该把自己的边界做清楚：awiki.ai 不需要重做所有动态本体平台能力；Product OSDK 也不应该直接取代 OntoFlow、知识图谱平台、数据中台、湖仓、GIS 平台或规则引擎。

更稳妥的定位是：

```text
OntoFlow 或其他动态本体平台，是 OSDK 的上游生产系统；
这些平台提供建模、加工、实例化、图关系和动作编排能力；
我们的 Product Compiler 把其中适合对外服务的能力裁剪成数据产品；
awiki.ai Agent 网络把这些上游能力注册成可操作 skill，让数据运营 Agent 可以协同生产、质检和发布。
```

![上游动态本体平台、Agent Skill 与 OSDK 产品层分工](assets/architecture-diagrams/upstream-ontology-skill-boundary.png)

```mermaid
flowchart LR
    subgraph Provider["数据提供方内部生产域"]
        Catalog["数据目录 / 湖仓 / GIS / 图数据库"]
        OntoFlow["OntoFlow / 动态本体平台<br/>建模、映射、实例化、动作编排"]
        Quality["质量检查 / 分类分级 / 血缘"]
        DOIR["DOIR / Product Projection<br/>中间表示与产品投影"]
    end

    subgraph Awiki["awiki.ai Agent 网络"]
        OpsAgent["数据运营 Agent"]
        SkillRegistry["Skill Registry<br/>建模、质检、编译、发布、审计"]
        Human["人工审批 / 复核"]
    end

    subgraph ProductLayer["我们的可信数据产品层"]
        Compiler["Product Compiler"]
        OSDKGen["OSDK / MCP / OpenAPI Generator"]
        Gateway["可信数据空间网关"]
    ...
    OntoFlow --> DOIR
    Quality --> DOIR
    OpsAgent --> SkillRegistry
    SkillRegistry --> OntoFlow
    SkillRegistry --> Quality
...
- 对数据提供方友好：它们可以继续使用已有数据平台或 OntoFlow 类平台，不需要为了接入 awiki.ai 推倒重来。
- 对 Agent 网络友好：Agent 不只是调用最终 OSDK，还能在提供方内部调用“建模、质检、编译、发布、审计”等 skill，形成真正的数据服务生产链。
- 对可信边界友好：上游平台可以复杂、灵活、全栈；对外暴露的 OSDK 必须收敛、授权、可审计、可验证。

所以我们要做的是“把上游动态本体平台生产出的能力产品化、可信化、Agent 化”，而不是和上游平台争夺同一层。

### 5.8 Demo 应该新增的 awiki.ai 视角
...
### 9.3 数据运营 Agent 的实施风险

数据运营 Agent 的技术风险不在“能不能让 Agent 调工具”，而在“工具权限如何分层、动作如何审计、关键节点如何人工介入”。如果做成 Agent 直接连库、直接改表、直接发布产品，风险会迅速超过收益。

建议采用四级实施路线：

| 阶段 | Agent 能做什么 | 技术风险 | 人工介入 |
| --- | --- | --- | --- |
| L1 辅助说明 | 读取产品文档、解释口径、生成 FAQ、整理审批材料 | 低 | 抽样复核 |
| L2 只读分析 | 读取元数据、质量指标、血缘、样例统计，提出建模和分类建议 | 中 | 建模负责人确认 |
| L3 受控生产 | 触发质量任务、生成 DOIR 草案、调用 Product Compiler、生成 OSDK 草案 | 中到高 | 发布前强制审批 |
| L4 有限自治运营 | 对低风险产品自动重跑质检、重编译、通知客户和登记 Receipt 验证结果 | 高 | 需要白名单、配额、回滚和审计 |

落地时应优先建设这些控制能力：

- Skill Registry：所有可调用工具都必须登记用途、输入输出、权限和风险级别。
- 最小权限凭证：Agent 不持有人的长期凭证，只拿短期、限定动作、可撤销的 token。
- 人工审批钩子：发布产品、扩大权限、批准特殊用途、外发结果、修改生产绑定必须进入人审。
- 审计回放：每个 Agent 动作记录输入、输出、版本、操作者、审批人和结果 hash。
- 沙箱验证：编译、测试、样例执行先在沙箱完成，不能直接影响生产 Runtime。

这意味着：数据运营 Agent 需要“更多全栈能力”，但这些能力应该被产品化成可治理的 skill，而不是把底层平台权限裸露给 Agent。
...
| 数据运营 Agent | 数据提供方内部的技术型 Agent，受控调用数据目录、动态本体平台、质量、编译、发布和审计工具 |
| OntoFlow / 动态本体平台 | OSDK 的上游生产系统，负责建模、映射、实例化、图关系和动作编排，可注册为 awiki.ai Agent 网络中的 skill |
